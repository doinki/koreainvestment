# [주식예약주문조회[v1\_국내주식-020]](https://apiportal.koreainvestment.com/apiservice-apiservice?/uapi/domestic-stock/v1/trading/order-resv-ccnl)
...
| output                    | 응답상세         | Array  | Y        |        |                             |
| output.rsvn_ord_seq       | 예약주문 순번    | String | N        | 10     |                             |
| output.rsvn_ord_ord_dt    | 예약주문주문일자 | String | N        | 8      |                             |
| output.rsvn_ord_rcit_dt   | 예약주문접수일자 | String | N        | 8      |                             |
| output.pdno               | 상품번호         | String | N        | 12     |                             |
| output.ord_dvsn_cd        | 주문구분코드     | String | N        | 2      |                             |
| output.ord_rsvn_qty       | 주문예약수량     | String | N        | 10     |                             |
| output.tot_ccld_qty       | 총체결수량       | String | N        | 10     |                             |
| output.cncl_ord_dt        | 취소주문일자     | String | N        | 8      |                             |
| output.ord_tmd            | 주문시각         | String | N        | 6      |                             |
| output.ctac_tlno          | 연락전화번호     | String | N        | 20     |                             |
| output.rjct_rson2         | 거부사유2        | String | N        | 200    |                             |
| output.odno               | 주문번호         | String | N        | 10     |                             |
| output.rsvn_ord_rcit_tmd  | 예약주문접수시각 | String | N        | 6      |                             |
| output.kor_item_shtn_name | 한글종목단축명   | String | N        | 60     |                             |
| output.sll_buy_dvsn_cd    | 매도매수구분코드 | String | N        | 2      |                             |
| output.ord_rsvn_unpr      | 주문예약단가     | String | N        | 19     |                             |
| output.tot_ccld_amt       | 총체결금액       | String | N        | 19     |                             |
| output.loan_dt            | 대출일자         | String | N        | 8      |                             |
| output.cncl_rcit_tmd      | 취소접수시각     | String | N        | 6      |                             |
| output.prcs_rslt          | 처리결과         | String | N        | 60     |                             |
| output.ord_dvsn_name      | 주문구분명       | String | N        | 60     |                             |
| output.tmnl_mdia_kind_cd  | 단말매체종류코드 | String | N        | 2      |                             |
| output.rsvn_end_dt        | 예약종료일자     | String | N        | 8      |                             |
