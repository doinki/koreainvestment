# [국내주식 재무비율 순위[v1\_국내주식-092]](https://apiportal.koreainvestment.com/apiservice-apiservice?/uapi/domestic-stock/v1/ranking/finance-ratio)
...
| output.cptl_op_prfi        | 총자본경상이익율       | String       | Y        | 92     |             |
| output.cptl_ntin_rate      | 총자본 순이익율        | String       | Y        | 92     |             |
| output.sale_totl_rate      | 매출액 총이익율        | String       | Y        | 92     |             |
| output.sale_ntin_rate      | 매출액 순이익율        | String       | Y        | 92     |             |
| output.bis                 | 자기자본비율           | String       | Y        | 92     |             |
| output.lblt_rate           | 부채 비율              | String       | Y        | 84     |             |
| output.bram_depn           | 차입금 의존도          | String       | Y        | 92     |             |
| output.rsrv_rate           | 유보 비율              | String       | Y        | 124    |             |
| output.grs                 | 매출액 증가율          | String       | Y        | 124    |             |
| output.op_prfi_inrt        | 경상 이익 증가율       | String       | Y        | 124    |             |
| output.bsop_prfi_inrt      | 영업 이익 증가율       | String       | Y        | 124    |             |
| output.ntin_inrt           | 순이익 증가율          | String       | Y        | 124    |             |
| output.equt_inrt           | 자기자본 증가율        | String       | Y        | 92     |             |
| output.cptl_tnrt           | 총자본회전율           | String       | Y        | 92     |             |
| output.sale_bond_tnrt      | 매출 채권 회전율       | String       | Y        | 92     |             |
| output.totl_aset_inrt      | 총자산 증가율          | String       | Y        | 92     |             |
| output.stac_month          | 결산 월                | String       | Y        | 2      |             |
| output.stac_month_cls_code | 결산 월 구분 코드      | String       | Y        | 2      |             |
| output.iqry_csnu           | 조회 건수              | String       | Y        | 10     |             |
