# [국내주식 시간외예상체결등락률 [국내주식-140]](https://apiportal.koreainvestment.com/apiservice-apiservice?/uapi/domestic-stock/v1/ranking/overtime-exp-trans-fluct)
...
| output.iscd_stat_cls_code           | 종목 상태 구분 코드            | String | Y        | 3      |             |
...
| output.ovtm_untp_antc_cntg_vrsssign | 시간외 단일가 예상 체결 대비   | String | Y        | 1      |             |
| output.ovtm_untp_antc_cntg_ctrt     | 시간외 단일가 예상 체결 대비율 | String | Y        | 82     |             |
| output.ovtm_untp_askp_rsqn1         | 시간외 단일가 매도호가 잔량1   | String | Y        | 12     |             |
| output.ovtm_untp_bidp_rsqn1         | 시간외 단일가 매수호가 잔량1   | String | Y        | 12     |             |
| output.ovtm_untp_antc_cnqn          | 시간외 단일가 예상 체결량      | String | Y        | 18     |             |
| output.itmt_vol                     | 장중 거래량                    | String | Y        | 18     |             |
