# [해외주식 현재가상세[v1\_해외주식-029]](https://apiportal.koreainvestment.com/apiservice-apiservice?/uapi/overseas-price/v1/quotations/price-detail)
...
| output.rsym    | 실시간조회종목코드 | String | Y        | 16     |                |
| output.pvol    | 전일거래량         | String | Y        | 14     |                |
| output.open    | 시가               | String | Y        | 12     |                |
| output.high    | 고가               | String | Y        | 12     |                |
| output.low     | 저가               | String | Y        | 12     |                |
| output.last    | 현재가             | String | Y        | 12     |                |
| output.base    | 전일종가           | String | Y        | 12     |                |
| output.tomv    | 시가총액           | String | Y        | 16     |                |
| output.pamt    | 전일거래대금       | String | Y        | 14     |                |
| output.uplp    | 상한가             | String | Y        | 12     |                |
| output.dnlp    | 하한가             | String | Y        | 12     |                |
| output.h52p    | 52주최고가         | String | Y        | 12     |                |
| output.h52d    | 52주최고일자       | String | Y        | 8      |                |
| output.l52p    | 52주최저가         | String | Y        | 12     |                |
| output.l52d    | 52주최저일자       | String | Y        | 8      |                |
| output.perx    | PER                | String | Y        | 10     |                |
| output.pbrx    | PBR                | String | Y        | 10     |                |
| output.epsx    | EPS                | String | Y        | 10     |                |
| output.bpsx    | BPS                | String | Y        | 10     |                |
| output.shar    | 상장주수           | String | Y        | 16     |                |
| output.mcap    | 자본금             | String | Y        | 16     |                |
| output.curr    | 통화               | String | Y        | 4      |                |
| output.zdiv    | 소수점자리수       | String | Y        | 1      |                |
| output.vnit    | 매매단위           | String | Y        | 6      |                |
| output.t_xprc  | 원환산당일가격     | String | Y        | 12     |                |
| output.t_xdif  | 원환산당일대비     | String | Y        | 12     |                |
| output.t_xrat  | 원환산당일등락     | String | Y        | 12     |                |
| output.p_xprc  | 원환산전일가격     | String | Y        | 12     |                |
| output.p_xdif  | 원환산전일대비     | String | Y        | 12     |                |
| output.p_xrat  | 원환산전일등락     | String | Y        | 12     |                |
| output.t_rate  | 당일환율           | String | Y        | 12     |                |
| output.p_rate  | 전일환율           | String | Y        | 12     |                |
| output.t_xsgn  | 원환산당일기호     | String | Y        | 1      | HTS 색상표시용 |
| output.p_xsng  | 원환산전일기호     | String | Y        | 1      | HTS 색상표시용 |
| output.e_ordyn | 거래가능여부       | String | Y        | 20     |                |
| output.e_hogau | 호가단위           | String | Y        | 8      |                |
| output.e_icod  | 업종(섹터)         | String | Y        | 40     |                |
| output.e_parp  | 액면가             | String | Y        | 12     |                |
| output.tvol    | 거래량             | String | Y        | 14     |                |
| output.tamt    | 거래대금           | String | Y        | 14     |                |
| output.etyp_nm | ETP 분류명         | String | Y        | 20     |                |
