# [선물옵션 정정취소주문[v1\_국내선물-002]](https://apiportal.koreainvestment.com/apiservice-apiservice?/uapi/domestic-futureoption/v1/trading/order-rvsecncl)
...
- 실전 TR ID: (주간 정정/취소) TTTO1103U (야간 정정/취소) (구) JTCE1002U (신) STTN1103U
- 모의 TR ID: (주간 정정/취소) VTTO1103U (야간은 모의투자 미제공)
...
| output                | 응답상세         | Array  | Y        |        |                                                  |
| output.ACNT_NAME      | 계좌명           | String | Y        | 60     | 계좌의 고객명                                    |
| output.TRAD_DVSN_NAME | 매매구분명       | String | Y        | 60     | 매도/매수 등 구분값                              |
| output.ITEM_NAME      | 종목명           | String | Y        | 60     | 주문 종목 명칭                                   |
| output.ORD_TMD        | 주문시각         | String | Y        | 6      | 주문 접수 시간                                   |
| output.ORD_GNO_BRNO   | 주문채번지점번호 | String | Y        | 5      | 계좌 개설 시 관리점으로 선택한 영업점의 고유번호 |
| output.ORGN_ODNO      | 원주문번호       | String | Y        | 10     | 정정 또는 취소 대상 주문의 일련번호              |
| output.ODNO           | 주문번호         | String | Y        | 10     | 접수한 주문(정정 또는 취소)의 일련번호           |
