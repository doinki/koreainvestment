# [해외주식 현재체결가[v1\_해외주식-009]](https://apiportal.koreainvestment.com/apiservice-apiservice?/uapi/overseas-price/v1/quotations/price)
...
- 실전 TR ID: HHDFS00000300
- 모의 TR ID: HHDFS00000300
...
| output.rsym | 실시간조회종목코드 | String | Y        | 16     | D+시장구분(3자리)+종목코드 예) DNASAAPL : D+NAS(나스닥)+AAPL(애플) [시장구분] NYS : 뉴욕, NAS : 나스닥, AMS : 아멕스 , TSE : 도쿄, HKS : 홍콩, SHS : 상해, SZS : 심천 HSX : 호치민, HNX : 하노이 |
| output.zdiv | 소수점자리수       | String | Y        | 1      |                                                                                                                                                                                                  |
| output.base | 전일종가           | String | Y        | 12     | 전일의 종가                                                                                                                                                                                      |
| output.pvol | 전일거래량         | String | Y        | 14     | 전일의 거래량                                                                                                                                                                                    |
| output.last | 현재가             | String | Y        | 12     | 당일 조회시점의 현재 가격                                                                                                                                                                        |
| output.sign | 대비기호           | String | Y        | 1      | 1 : 상한 2 : 상승 3 : 보합 4 : 하한 5 : 하락                                                                                                                                                     |
| output.diff | 대비               | String | Y        | 12     | 전일 종가와 당일 현재가의 차이 (당일 현재가-전일 종가)                                                                                                                                           |
| output.rate | 등락율             | String | Y        | 12     | 전일 대비 / 당일 현재가 \* 100                                                                                                                                                                   |
| output.tvol | 거래량             | String | Y        | 14     | 당일 조회시점까지 전체 거래량                                                                                                                                                                    |
| output.tamt | 거래대금           | String | Y        | 14     | 당일 조회시점까지 전체 거래금액                                                                                                                                                                  |
| output.ordy | 매수가능여부       | String | Y        | 20     | 매수주문 가능 종목 여부                                                                                                                                                                          |
