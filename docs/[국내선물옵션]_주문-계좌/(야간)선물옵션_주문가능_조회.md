# [(야간)선물옵션 주문가능 조회 [국내선물-011]](https://apiportal.koreainvestment.com/apiservice-apiservice?/uapi/domestic-futureoption/v1/trading/inquire-psbl-ngt-order)
...
- 모의 DOMAIN: 모의투자 미지원
- 실전 TR ID: (구) JTCE1004R (신) STTN5105R
- 모의 TR ID: 모의투자 미지원
...
| output.max_ord_psbl_qty | 최대주문가능수량 | String | Y        | 19     | 최대주문가능수량 (신규 TR 미사용 필드) |
| output.tot_psbl_qty     | 최대주문가능수량 | String | Y        | 19     |                                        |
| output.lqd_psbl_qty     | 청산가능수량     | String | Y        | 19     | 청산가능수량                           |
| output.lqd_psbl_qty_1   | 청산가능수량     | String | Y        | 19     | 신규 TR 사용 필드                      |
| output.ord_psbl_qty     | 주문가능수량     | String | Y        | 19     |                                        |
| output.bass_idx         | 기준지수         | String | Y        | 23     | 신규 TR 사용 필드                      |
