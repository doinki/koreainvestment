# [관심종목(멀티종목) 시세조회 [국내주식-205]](https://apiportal.koreainvestment.com/apiservice-apiservice?/uapi/domestic-stock/v1/quotations/intstock-multprice)
...
| output.kospi_kosdaq_cls_name    | 코스피 코스닥 구분 명      | String | Y        | 10     |             |
| output.mrkt_trtm_cls_name       | 시장 조치 구분 명          | String | Y        | 10     |             |
| output.hour_cls_code            | 시간 구분 코드             | String | Y        | 1      |             |
| output.inter_shrn_iscd          | 관심 단축 종목코드         | String | Y        | 16     |             |
| output.inter_kor_isnm           | 관심 한글 종목명           | String | Y        | 40     |             |
| output.inter2_prpr              | 관심2 현재가               | String | Y        | 11     |             |
| output.inter2_prdy_vrss         | 관심2 전일 대비            | String | Y        | 11     |             |
| output.prdy_vrss_sign           | 전일 대비 부호             | String | Y        | 1      |             |
| output.prdy_ctrt                | 전일 대비율                | String | Y        | 82     |             |
| output.acml_vol                 | 누적 거래량                | String | Y        | 18     |             |
| output.inter2_oprc              | 관심2 시가                 | String | Y        | 11     |             |
| output.inter2_hgpr              | 관심2 고가                 | String | Y        | 11     |             |
| output.inter2_lwpr              | 관심2 저가                 | String | Y        | 11     |             |
| output.inter2_llam              | 관심2 하한가               | String | Y        | 11     |             |
| output.inter2_mxpr              | 관심2 상한가               | String | Y        | 11     |             |
| output.inter2_askp              | 관심2 매도호가             | String | Y        | 11     |             |
| output.inter2_bidp              | 관심2 매수호가             | String | Y        | 11     |             |
| output.seln_rsqn                | 매도 잔량                  | String | Y        | 12     |             |
| output.shnu_rsqn                | 매수2 잔량                 | String | Y        | 12     |             |
...
| output.inter2_prdy_clpr         | 관심2 전일 종가            | String | Y        | 11     |             |
| output.oprc_vrss_hgpr_rate      | 시가 대비 최고가 비율      | String | Y        | 84     |             |
| output.intr_antc_cntg_vrss      | 관심 예상 체결 대비        | String | Y        | 11     |             |
| output.intr_antc_cntg_vrss_sign | 관심 예상 체결 대비 부호   | String | Y        | 1      |             |
| output.intr_antc_cntg_prdy_ctrt | 관심 예상 체결 전일 대비율 | String | Y        | 72     |             |
| output.intr_antc_vol            | 관심 예상 거래량           | String | Y        | 18     |             |
| output.inter2_sdpr              | 관심2 기준가               | String | Y        | 11     |             |
